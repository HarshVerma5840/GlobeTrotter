...
- **B9** [P1] Public share routes (CONTRACTS §6), token-scoped, no auth
  dependency.
- **B10** [P1] Smart Trip Assistant: `services/autoplan.py` + `POST
  /trips/{id}/auto-plan` (CONTRACTS §7.1).
- **B11** [P1] `services/feasibility.py` (Haversine + threshold) and the
  computed fields exposed on stop responses (CONTRACTS §7.2).
...
- **F13** [P1] **Route Map component** (`react-leaflet`) — pins,
  connecting lines, feasibility highlight, reading the fields B11 exposes.
  Coordinate the online-tiles-vs-offline-SVG decision with Integration
  (I4) before building the rendering half; the data it reads is identical
  either way (CONTRACTS §7.2), so the component logic can start now.
...
- **I4** [P1] Decide map-rendering mode (online Leaflet/OSM tiles vs.
  offline static-SVG fallback) based on confirmed venue connectivity —
  needed before F13's rendering half, not before its logic half.
...
- **Q6** [P1] Verify the feasibility flag triggers on a deliberately bad
  ...
  *not* trigger on a reasonable one.

...
import type { TripCard } from "../../data/profileData";
...
function PreplannedCard({ trip }: { trip: TripCard }) {
  return (
...
function ArchiveCard({ trip }: { trip: TripCard }) {
  return (
    <article className="group cursor-pointer bg-surface-container rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-500 ease-out flex flex-col">
...
export default function TripGrid({
  ...
  return (
    <section className={`flex flex-col gap-12 ${variant === "archive" ? "mb-24" : ""}`}>
      ...
      {/* Card grid */}
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 ${variant === "archive" ? "opacity-90" : ""}`}>
        {trips.map((trip) =>
          variant === "preplanned" ? (
